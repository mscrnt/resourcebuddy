...
class ResourceSpaceCache:
    ...
    def _init_database(self):
        ...
        if os.path.exists('cache_schema.sql'):
            ...
            raise FileNotFoundError("cache_schema.sql not found")
    ...
    def fetch_and_cache_file(self, resource_id: int, 
        ...
        try:
            ...
            if file_url:
                ...
                # Construct API call to get resource file
                api_params = {
                ...
                response = requests.get(self.rs_api_url, params=api_params, timeout=30)
                response.raise_for_status()
                file_url = response.text.strip()
                
                if not file_url:
                    logger.error(f"No file URL returned for resource {resource_id}")
                    return None
